...
Client.prototype.init = function(options, callback) {
  ...
  this.socket.addListener('message', async function(msg, rinfo) {
    // Ignore own messages and false formats
    if (!Buffer.isBuffer(msg)) return
    if(await utils.getHostIPs() === rinfo.address) return
    ...
    if (parsedMsg instanceof Error) {
      console.warn('LIFX Client invalid packet header error');
      console.warn('Packet: ', msg.toString('hex'));
  ...
  if (this.messagesQueue.length > 0) {
    ...
    if (msg.transactionType === constants.PACKET_TRANSACTION_TYPES.ONE_WAY) {
      ...
      if (msg.timesSent < this.resendMaxTimes) {
        if (Date.now() > (msg.timeLastSent + this.resendPacketDelay*2)) {
          this.socket.send(msg.data, 0, msg.data.length, this.port, msg.address);
          msg.timesSent += 1;
