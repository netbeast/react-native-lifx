...
Light.prototype.getState = function(callback) {
  ...
  const timer = setTimeout(() => {
    return callback(new Error('[LIFX] getState request timeout'), null);
  }, 8000);
 ...
 * @param {Object} color
 * {
 *   hue                              color hue from 0 - 360 (in °)
 *   saturation                       color saturation from 0 - 100 (in %)
 *   brightness                       color brightness from 0 - 100 (in %)
 *   [kelvin=3500]                    color kelvin between 2500 and 9000
 * }
